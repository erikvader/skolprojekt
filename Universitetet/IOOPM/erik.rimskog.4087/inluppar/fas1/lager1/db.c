...
struct undo_action{
   enum {ADD, REMOVE, EDIT} type; //type of action we did
   union {
      struct{ item_t *removed; }; //the item we removed
      struct{ K added; }; //the name of the item we added
      struct{ item_t *original; }; //pointer to what we edited and pointer to the original version
   };
};
...
// returns a pointer to a newly allocated shelf_t on the heap with name
// 'name' (no strdup) and amount 'num'.
...
// frees a shelf
void delete_shelf(shelf_t *s){
   free(s->name);
   free(s);
}

// returns a pointer to a newly allocated item_t on the heap.
// it has the name 'name' (no strdup), description 'desc' (no strdup) and price 'price'.
...
// frees everything in the item_t t
void delete_item(item_t *t){
   free(t->name);
   free(t->desc);
   list_delete(t->shelves, (list_action) delete_shelf);
   free(t);
}

// returns a pointer to a complete copy of s
shelf_t* copy_shelf(shelf_t *s){
   return make_shelf(strdup(s->name), s->num);
}

// returns a pointer to a complete copy of 'item'
item_t* copy_item(item_t *item){
   ...
   while(list_iterator_has_next(ite)){
      list_append(copy->shelves, copy_shelf(*list_iterator_next(ite)));
   }
   free(ite);
   return copy;
}
...
void undo_free_action(undo_action_t *a){
   if(a->type == REMOVE){
      delete_item(a->removed);
   }else if(a->type == ADD){
      free(a->added);
   }else if(a->type == EDIT){
      delete_item(a->original);
   }
   free(a);
}

// adds an add action to the global variable last_action
// removing the old one.
// 'key' is a copy of the name of the item that was added.
// undo_undo will remove the item with the same name as 'key'
void undo_add_add(K key){
   if(last_action != NULL) undo_free_action(last_action);
   last_action = undo_make_action();
   last_action->type = ADD;
   last_action->added = strdup(key);
}

// adds an remove action to the global variable last_action
// removing the old one.
// 'removed' is a pointer to the removed item.
// undo_undo will reinsert this item.
void undo_add_remove(item_t *removed){
   if(last_action != NULL) undo_free_action(last_action);
   last_action = undo_make_action();
   last_action->type = REMOVE;
   last_action->removed = removed;
}

// adds an edit action to the global variable last_action
// removing the old one.
// 'copy' is a copy of how the item looked like before the edit.
// undo_undo will remove the item with the same name as 'copy' (copy->name)
// and insert 'copy'
void undo_add_edit(item_t *copy){
   if(last_action != NULL) undo_free_action(last_action);
   last_action = undo_make_action();
   last_action->type = EDIT;
   last_action->original = copy;
}

// takes the value in 'last_action' and reverts what it did.
// Setting it to NULL afterwards.
void undo_undo(tree_t *tree){
   if(last_action != NULL){
      if(last_action->type == REMOVE){
         printf("Lägger till %s igen\n", last_action->removed->name);
         tree_insert(tree, last_action->removed->name, last_action->removed);
         free(last_action);
      }else if(last_action->type == ADD){
         printf("Du tar nu bort %s\n", last_action->added);
         delete_item(tree_remove(tree, last_action->added));
         undo_free_action(last_action);
      }else if(last_action->type == EDIT){
         printf("Du revertar en redigering på %s\n", last_action->original->name);
         delete_item(tree_remove(tree, last_action->original->name));
         tree_insert(tree, last_action->original->name, last_action->original);
         free(last_action);
      }
      last_action = NULL;
   }
...
// checks whether 'str' is a correctly formatted shelf name.
...
// Checks whether a shelf with the name 'name' is already used by another item.
// If it is NOT used, then NULL is returned.
// If it is in use, then a pointer to the name of the item that uses it is returned.
char* shelf_in_use(tree_t *t, char *name){
   T* elements = tree_elements(t);
   int size = tree_size(t);
   char *toreturn = NULL;
   for(int i = 0; i < size; i++){
      item_t *cur = ((item_t*)elements[i]);
      list_iterator_t *ite = list_get_iterator(cur->shelves);
      while(list_iterator_has_next(ite)){
         shelf_t *she = (shelf_t*)(*list_iterator_next(ite));
         if(strcmp(she->name, name) == 0){
            toreturn = cur->name;
            i = size;
            break;
         }
