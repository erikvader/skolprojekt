...
public class Twitterish {
    ...
    private static class Client {
        ...
        private void addFriend() {
            ...
            while(true){
               try{
                  ...
                  if(choice >= 0 && choice < knownUsers.length){
                     break;
                  }else{
                     System.out.println("din siffra är lite fel");
                  }
        ...
        private void unIgnoreFriend() {
            if (this.loggedInUser.hasIgnored() == false) {
                System.out.println("You haven't ignored anyone. Try to ignore a friend first.");
                return;
            }
        ...
        private void syncWithServer() {
            ...
            if (o instanceof SyncResponse) {
                ...
                TreeSet<Account> ku = (TreeSet<Account>) this.knownUsers;
                Iterator<Account> newUsers = ((SyncResponse) o).getUsers().iterator();
                while(newUsers.hasNext()){
                   Account a = newUsers.next();
                   Account local = ku.floor(a);
                   if(local != null && local.equals(a)){
                ...
                for (Post p : ((SyncResponse) o).getPosts()) {
                   Account local = ku.floor(p.getPoster());
                   if(local != null && local.equals(p.getPoster())){
                      newPost(new Post(p.getGlobalPostId(), local, p.getContent()));
                   }
                }
            } else {
        ...
        private void updateFeed() {
            System.out.println(this.feed.renderAll(loggedInUser));
        }
