...
public class Solnedgang_new {

	public static void main(String[] args) {
		new Solnedgang_new().run();
	}
	...
	public void findShortestPath(){
		...
		ArrayList<Node> openListFalse = new ArrayList<>();
		ArrayList<Node> closedList = new ArrayList<>();
		
		openList.add(nodes[start.x][start.y]);
		
		while(openList.size() > 0 || openListFalse.size() > 0){
			...
			*/
			if(openList.size() > 0){
				...
				Point h;
				Node n;
				for(int i = openListFalse.size()-1; i >= 0; i--){
					n = openListFalse.get(i);
					h = getClosestHouseUnder(n.getPoint());
					if(n.y - (h.y+1) <= curTime){
						n.dayTime = true;
						openList.add(n);
						openListFalse.remove(i);
					}
				}
				continue;
			}
			...
			for(int x = -1; x <= 1; x++){
				for(int y = -1; y <= 1; y++){
					...
					if(newX >= 0 && newX < width && newY >= 0 && newY < (height+curTime-1)){
						...
						ArrayList<Node> curList = neighbour.dayTime ? openList : openListFalse;
						
						if(!curList.contains(neighbour) || newG < neighbour.g){
							neighbour.parent = current;
							neighbour.g = newG;
							neighbour.h = getHeuristic(neighbour);
							if(!curList.contains(neighbour)){
								curList.add(neighbour);
							}
						}
					}
	...
	class Node{
		...
		private Solnedgang_new getOuterType() {
			return Solnedgang_new.this;
		}
