...
public class Problem107 {
	...
	int vertices = 40;
	String[] txtFile = {"-,-,-,427,668,495,377,678,-,177,-,-,870,-,869,624,300,609,131,-,251,-,-,-,856,221,514,-,591,762,182,56,-,884,412,273,636,-,-,774",
	...
	public void run(){
		...
		for(int y = 0; y < vertices; y++){
			String line = txtFile[y];
			String[] eles = line.split(",");
			for(int x = 0; x < y; x++){
				if(!eles[x].equals("-")){
					int w = Integer.parseInt(eles[x]);
		...
		//kör minsta uppspännande träd
		for(int i = 0; i < edges.size(); i++){
			Edge toTest = edges.get(i);
			totalCost += toTest.weight;
			nodes.get(toTest.from).neighbours.add(nodes.get(toTest.to));
			nodes.get(toTest.to).neighbours.add(nodes.get(toTest.from));
			if(testIfValid(nodes.get(toTest.from))){
				newCost += toTest.weight;
			}else{
				ArrayList<Node> temp = nodes.get(toTest.from).neighbours;
				temp.remove(temp.size()-1);
				temp = nodes.get(toTest.to).neighbours;
				temp.remove(temp.size()-1);
			}
		}
		
		System.out.println(totalCost-newCost);
	}
	
	public boolean testIfValid(Node startPoint){
		
		HashSet<Node> visited = new HashSet<Node>();
		
		//visited.add(startPoint);
		return search(visited, null, startPoint);
		
	}
	
	public boolean search(HashSet<Node> visited, Node last, Node cur){
		if(!visited.add(cur)){ //vi har redan besökt den en gång och vi har därmed en "loop"
			return false;
		}
		
		//sök och se ifall vi kommer tillbaka till samma nod igen
		for(int j = 0; j < cur.neighbours.size(); j++){
			Node toVisit = cur.neighbours.get(j);
			if(last != null && last.equals(toVisit)) continue;
			
			if(!search(visited, cur, toVisit)) return false;
		}
	...
	class Node{
		...
		private Problem107 getOuterType() {
			return Problem107.this;
		}
		@Override
		public String toString() {
			return "Node [num=" + num + "]";
